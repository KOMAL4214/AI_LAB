...
struct NodeBFS{
    ...
    int cos;
    vector<string> path;
    NodeBFS(string c,int heuristic,int co,vector<string> p){
        city=c; h=heuristic; cos=co; path=p;
    }
};
struct NodeAStar{
    string city;
    int g,h,f;
    vector<string> path;
    NodeAStar(string c,int gCost,int hCost,vector<string> p){
        city=c; g=gCost; h=hCost; f=g+h; path=p;
    }
};
class PQ_BFS{
    vector<NodeBFS> data;
...
public:
    ...
    void pop(){ frontIndex++; }
    NodeBFS top(){ return data[frontIndex]; }
    bool empty(){ return frontIndex>=data.size(); }
};
class PQ_AStar{
    vector<NodeAStar> data;
    int frontIndex;
public:
    PQ_AStar(){ frontIndex=0; }
    void push(NodeAStar x){
        data.push_back(x);
        int i=data.size()-1;
    ...
    void pop(){ frontIndex++; }
    NodeAStar top(){ return data[frontIndex]; }
    bool empty(){ return frontIndex>=data.size(); }
};
...
void bestFirstSearch(vector<vector<int>>&graph,
    ...
    pq.push(NodeBFS(start,heuristic[startIndex],0,{start}));
    int pathsExplored=0;
    cout << "\nGREEDY BEST FIRST SEARCH\n";
    while(!pq.empty()){
        NodeBFS current=pq.top();
        pq.pop();
        ...
        cout<<" | h(n): "<<current.h;
        cout<<" | g(n): "<<current.cos<<"\n";
        if(current.city==goal){
            cout<<"\nGoal Reached!\n";
            cout<<"Total cost: "<<current.cos<<"\n";
            cout<<"Paths explored: "<<pathsExplored<<"\n";
            return;
        }
        for(int v=0;v<n;v++){
            if(graph[u][v]!=1e9 && !visited[v]){
                ...
                int newcost=current.cos+graph[u][v];
                pq.push(NodeBFS(cities[v],heuristic[v],newcost,newPath));
            }
        }
    }
}
void aStarSearch(vector<vector<int>>&graph,
                 vector<string>&cities,
                 vector<int>&heuristic,
                 string start,string goal){
    int n=cities.size();
    ...
    PQ_AStar pq;
    int startIndex=getIndex(cities,start);
    pq.push(NodeAStar(start,0,heuristic[startIndex],{start}));
    int pathsExplored=0;
    cout << "\nA* SEARCH\n";
    while(!pq.empty()){
        ...
        for(int v=0;v<n;v++){
            if(graph[u][v]!=1e9 && !visited[v]){
                ...
                vector<string> newPath=current.path;
                newPath.push_back(cities[v]);
                pq.push(NodeAStar(cities[v],newG,heuristic[v],newPath));
            }
        }
    }
}
